...
// GDK modifier bit masks (GdkModifierType), mirroring GtkInterop's private constants.
private let stateCtrl: UInt32 = 1 << 2
private let stateShift: UInt32 = 1 << 0
private let stateAlt: UInt32 = 1 << 3
...
struct LatinKeyvalTests {
    ...
    func latinScriptEarlyOut(keyval: UInt32) {
        ...
        let spy = TranslatorSpy()
        let result = latinKeyval(keyval, keycode: 34, state: 4,
    ...
    func translatorReceivesKeycodeAndState() {
        let spy = TranslatorSpy()
        _ = latinKeyval(cyrillicIeLower, keycode: 28, state: 5,
                        translate: fakeTranslator([0: keyvalT], spy: spy))
...
struct IsInterruptKeyTests {
    ...
    func escapeIsInterrupt() {
        ...
        #expect(isInterruptKey(keyval: keyvalEscape, keycode: 9, state: stateCtrl, translate: fakeTranslator([:])))
    }

    @Test("sole-Ctrl+C is an interrupt without invoking the translator (ASCII early-out)")
    func ctrlCIsInterrupt() {
        let spy = TranslatorSpy()
        #expect(isInterruptKey(keyval: keyvalC, keycode: 54, state: stateCtrl,
                               translate: fakeTranslator([:], spy: spy)))
        #expect(spy.calls == 0)
    }

    @Test("Ctrl+C with an extra modifier is not an interrupt and never translates",
          arguments: [stateCtrl | stateShift, stateCtrl | stateAlt])
    ...
    func ctrlCyrillicEsIsInterrupt() {
        #expect(isInterruptKey(keyval: cyrillicEs, keycode: 54, state: stateCtrl,
...
struct ResolveShortcutKeyTests {
    ...
    func cyrillicResolvesToLatinChord() {
        let resolution = resolveShortcutKey(keyval: cyrillicIeLower, keycode: 28, state: stateCtrl,
    ...
    func arrowRoutesToFallback() {
        let resolution = resolveShortcutKey(keyval: keyvalLeft, keycode: 113, state: stateCtrl | stateShift,
                                            translate: fakeTranslator([:]))
